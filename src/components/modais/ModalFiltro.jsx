...
function ModalFiltro({ isOpen, onClose, columns, onSave }) {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div className="bg-white rounded-md w-[1300px] h-[600px] flex flex-col">
                {/* Cabeçalho */}
                <div className="w-full bg-custom-azul-escuro flex justify-between items-center text-white p-5">
                    <h5 className="font-bold text-2xl">FILTROS</h5>
                    <button
                        className="rounded-full w-8 h-8 flex justify-center items-center bg-[#0A7F8E] hover:bg-[#00AAB5] transition-colors duration-300"
                        onClick={onClose}
                ...
                <div className="flex flex-1 m-8">
                    ...
                    <div className="flex flex-col items-center mx-5">
                        ...
                        ].map(({ onClick, bg, label, icon }, idx) => (
                            <div key={idx} className="relative group">
                                <button
                                    className={`rounded-full ${bg} hover:bg-opacity-75 active:bg-opacity-100 w-10 h-10 my-3 flex justify-center items-center`}
                                    onClick={onClick}
                                    aria-label={label}
                                >
                                    {icon}
                                ...
                                {/* Tooltip */}
                                <div className="absolute bottom-full mb-2 px-2 py-1 bg-gray-800 text-white text-xs rounded-md opacity-0 group-hover:opacity-100 transition-opacity duration-300 whitespace-nowrap">
                                    {label}
                                </div>
                            </div>
                        ))}
                    ...
                    <div className="flex-1 overflow-y-auto ml-8">
                        ...
                        <div className="max-h-[322px] overflow-y-auto">
                            <table className="min-w-full table-fixed bg-white border-2 border-custom-azul-escuro w-full">
                                <thead>
                                    <tr>
                ...
                {/* Rodapé com Botões */}
                <div className="flex justify-end m-2 mx-5 mb-5">
                    {[
                        { onClick: onClose, bg: 'bg-[#6c757d]', label: 'Cancelar', hoverBg: '#5a6268' },
                        { onClick: handleSave, bg: 'bg-[#0A7F8E]', label: 'Salvar', hoverBg: '#00AAB5' },
                    ].map(({ onClick, bg, label, hoverBg }, idx) => (
                        <button
                            key={idx}
                            className={`font-bold ${bg} border-none text-white rounded px-5 py-2.5 text-base cursor-pointer mr-2 flex items-center justify-center transition-colors duration-300 hover:bg-[${hoverBg}]`}
                            onClick={onClick}
                            aria-label={label}
                            title={label}
                        >
