...
import ModalModal from "./ModalModal"; // Certifique-se de que o caminho está correto
    ...
    const handleSave = () => {
        const condicoes = addedCampos.map((campo) => {
            ...
            if (!valor || !ordenacao) {
                ...
                setModalType("ALERTA");
                setModalOpen(true);
                return null; // Interrompe a criação de condições
            } else {
                return `${campo.value} ${ordenacao} '${valor}'`;
            }
        });
    
        const condicoesString = `${condicoes.filter(Boolean).join(' AND ')}`; // Remove valores nulos e junta as condições
    
        // Chama a função onSave do componente pai com o condicoesString
        if (onSave && condicoesString) {
