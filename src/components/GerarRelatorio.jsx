...
function GerarRelatorio({ selectedColumns }) {
    ...
    const fetchData = async (columns) => {
        try {
            const response = await fetch('http://localhost:8080/procurar/cliente', {
                method: 'GET',
                headers: {
            ...
            return columns.map(column => ({
                column,
                values: data.map(row => row[columns.indexOf(column)])
            }));
        } catch (error) {
    ...
    const handleGenerateReport = async () => {
        ...
        if (data.length > 0) {
            ...
            // Desenhar os títulos das colunas no cabeçalho
            selectedColumns.forEach((column, index) => {
                ctx.fillText(column, 20 + index * 200, 55); // Ajusta a posição com base no index
            });
